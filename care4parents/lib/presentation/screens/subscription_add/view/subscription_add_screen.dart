...
class _SubscriptionAddScreenState extends State<SubscriptionAddScreen> {
  ...
  SubscriptionAddBloc _subscriptionAddBloc;

  @override
  initState() {
    super.initState();
    _subscriptionAddBloc = getItInstance<SubscriptionAddBloc>();
    init();
  }

  Future<void> init() async {
    await _subscriptionAddBloc.getPackageList().then((packages) {
      if (packages != null) {
  ...
  Widget build(BuildContext context) {
    ...
    return isLoading
        ...
        : BlocProvider(
            create: (context) => _subscriptionAddBloc,
            child: Builder(
              builder: (context) {
                final loadingFormBloc =
                    BlocProvider.of<SubscriptionAddBloc>(context);
                return Theme(
                  ...
                  child: Scaffold(
                    ...
                    body: SafeArea(
                      child:
                          FormBlocListener<SubscriptionAddBloc, String, String>(
                        ...
                        child: StepperFormBlocBuilder<SubscriptionAddBloc>(
                          type: _type,
  ...
  FormBlocStep _personalStep(SubscriptionAddBloc subscriptionAddFormBloc) {
    return FormBlocStep(
      title: Text(''),
      content: Column(
  ...
  FormBlocStep _accountStep(SubscriptionAddBloc subscriptionAddFormBloc) {
  ...
  FormBlocStep _packageStep(SubscriptionAddBloc subscriptionAddFormBloc) {
    double height = assignHeight(context: context, fraction: 1.0);
  ...
  FormBlocStep _finalSummaryStep(SubscriptionAddBloc subscriptionAddFormBloc) {
    double height = assignHeight(context: context, fraction: 1.0);
  ...
  Future<void> onPressAppy(code) async {
    if (_couponList.contains(code)) {
      ...
      await _subscriptionAddBloc.checkCouponValid().then((result) {
        print(result);
        if (result != null) {
          setState(() {
